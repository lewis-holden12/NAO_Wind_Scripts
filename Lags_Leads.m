...
wind_speed = [prctile(wind_data_QC(:,3), 50) 34 41 prctile(wind_data_QC(:,3), 99.5)];
...
for k = 1:years;
    decembers(k) = datenum([1948+k 03 01 00 00 00]);
    januarys(k) = datenum([1948+k 04 01 00 00 00]);
...
for k = 1:20;
    [oct_r_cell{k}, oct_p_cell{k}] = corrcoef(hourscell{k}(1:67), oct_nao(1:67,2));
end
...
for k = 1:20;
    [nov_r_cell{k}, nov_p_cell{k}] = corrcoef(hourscell{k}(1:67), nov_nao(1:67,2));
end
...
for k = 1:20;
    [dec_r_cell{k}, dec_p_cell{k}] = corrcoef(hourscell{k}(1:67), dec_nao(1:67,2));
end
...
for k = 1:20;
    [jan_r_cell{k}, jan_p_cell{k}] = corrcoef(hourscell{k}(1:66), jan_nao(2:67,2));
end
...
for k = 1:20;
    [feb_r_cell{k}, feb_p_cell{k}] = corrcoef(hourscell{k}(1:66), feb_nao(2:67,2));
end
...
mar_nao = csvread('MAR_NAO.csv');
mar_r_cell = cell(1,20);
feb_p_cell = cell(1,20);
mar_r = zeros(1,20);
mar_p = zeros(1,20);

for k =1:20;
    [mar_r_cell{k}, mar_p_cell{k}] = corrcoef(hourscell{k}(1:66), mar_nao(2:67));
end

for k = 1:20;
    mar_r(k) = mar_r_cell{k}(1,2);
    mar_p(k) = mar_p_cell{k}(1,2);
end
